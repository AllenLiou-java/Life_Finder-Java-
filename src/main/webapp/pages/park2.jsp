...
<body>
    ...
    <div style="width: 80%; margin: 0 auto; position: relative;">
	        ...
	        <table class="table table-bordered table-hover">
	            ...
	            <tbody>
	            	<c:forEach items="${lists.rows }" var="row" varStatus="status">
		                <tr>
		                    <td>${row.name }</td>
		                    <td>${row.lat }</td>
		                    <td>${row.lng }</td>
	...
	<c:if test="${pageNos>1 }">
		<a href="/controller/ParkServlet?pageNos=1" >首頁</a>
		<a href="/controller/ParkServlet?pageNos=${pageNos-1 }">上一頁</a>
	</c:if>
	<c:if test="${pageNos <recordCount }">
		<a href="/controller/ParkServlet?pageNos=${pageNos+1 }">下一頁</a>
		<a href="/controller/ParkServlet?pageNos=${recordCount }">末頁</a>
	</c:if>
